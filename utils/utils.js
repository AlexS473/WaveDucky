...
async function loadObj2(device, url) {
    ...
    for (let n of obj.result.models[0].vertexNormals) {
        normals.push(n.x);
        normals.push(n.y);
        normals.push(n.z);
    }

    for (let tc of obj.result.models[0].textureCoords) {
        texCoords.push(tc.u);
        texCoords.push(tc.v);
        texCoords.push(tc.w);
    }
    ...
    return {
        positionBuffer, normalBuffer, texCoordBuffer, vertexCount, center: [(minX + maxX) * 0.5, (minY + maxY) * 0.5, (minZ + maxZ) * 0.5],
        radius: Math.max(Math.max(maxX - minX, maxY - minY), maxZ - minZ) * 0.5
    }
}

async function shaderModuleFromCode(device, filePath) {
